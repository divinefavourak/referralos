...
### Key Architecture and Subsystems Implemented

#### 1. Relational and Geospatial Infrastructure (PostgreSQL 16 + PostGIS)
- **Migration and Schema (`backend/migrations/001_initial_schema.sql`):** 9 normalized relational tables (`facilities`, `capacity_resources`, `patients`, `referrals`, `matches`, `reservations`, `transport_assignments`, `tracking_events`, `audit_logs`).
- **PostGIS Spatial Modeling:** Integrated geography indexing for regional facility coordinates, road detour compensation factor (1.32x), and emergency ambulance speed models.
...
- **Phase 1 (Binary Hard Constraints):** Enforces binary validity. Evaluates operating theatre sterilization states, blood bank inventory levels (e.g. O- PRBC units), and required clinical specialties. Facilities failing any hard requirement (such as Hospital A's offline theatre) are rejected immediately.
- **Phase 2 (MCDA Composite Scoring):** Computes multi-criteria score balancing travel duration, resource buffer depth, historical handover latency, and facility congestion penalties.
...
#### 6. Real-Time Telematics and Clinical Handshake
...
- Clean conventional commit history with zero monolithic blobs.
